package org.IFInternal.security;
...
public class CryptographicHelper {
	...
	private final String DEFAULT_DOMAIN_CONFIG_DIRECTORY = System
			.getProperty("user.dir");
	private final String DEFAULT_KEYSTORE_NAME = "keystore.jks";
	private final char[] DEFAULT_KEYSTORE_PASSWORD = new char[] { 'c', 'h',
			'a', 'n', 'g', 'e', 'i', 't' };
	...
	private void doInitializeCryptographicHelper() {
		try {
			FileInputStream keyStoreStream = new FileInputStream(
					DEFAULT_DOMAIN_CONFIG_DIRECTORY
			...
			cert = (X509Certificate) keyStore
					.getCertificate(DEFAULT_CERTIFICATE_ALIAS);
			publicKey = cert.getPublicKey();
			privateKey = (PrivateKey) keyStore.getKey(
					DEFAULT_CERTIFICATE_ALIAS, DEFAULT_KEYSTORE_PASSWORD);
			...
			for (int i = 0; i < DEFAULT_IV_LENGTH; i++) {
				iv[i] = certPrivateKey[DEFAULT_PRIVATEKEY_LENGTH
						- DEFAULT_IV_LENGTH - i];
			}
	...
	public String doAESEncryptString(String stringToEncrypt, byte[] key,
			byte[] iv) {
		...
		try {
			...
			SecretKeySpec actualKey = new SecretKeySpec(key,
					DEFAULT_CIPHER_NAME);
			IvParameterSpec actualIV = new IvParameterSpec(iv, 0,
					DEFAULT_IV_LENGTH);
			cipher.init(Cipher.ENCRYPT_MODE, actualKey, actualIV);

			byte[] encryptedBuffer = cipher.doFinal(stringToEncrypt
					.getBytes(DEFAULT_CHARSET));
			encryptedString = new String(encryptedBuffer, DEFAULT_CHARSET);
		} catch (NoSuchAlgorithmException noSuchAlgoEx) {
			System.err.println("********** NoSuchAlgorithmException: "
					+ DEFAULT_CIPHER_ALGORITHM_NAME);
	...
	public String doAESDecryptString(String stringToDecrypt, byte[] key,
			byte[] iv) {
		...
		try {
			...
			byte[] decryptedBuffer = cipher.doFinal(stringToDecrypt
					.getBytes(DEFAULT_CHARSET));
